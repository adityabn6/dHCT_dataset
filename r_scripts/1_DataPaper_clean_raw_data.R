...
path <- "/home/nawatsw/UMM/BMT"
save_folder <- file.path(path, "DataPaper/dataset")
...
readRDS(paste0(path, "/participant_data.rds"))

# function

clean_data <- function(prtcpt_type, time_point) {

  data_path <- paste0(path, "/dataset/raw/Roadmap_2_", prtcpt_type, "_", time_point, ".csv")
...
for (prtcpt_type in c("Patients", "Caregivers")) {
  for (time_point in c("Baseline", "Day30", "Day120")) {
    ...
    save_path <- paste0(save_folder, "/Roadmap_2_", prtcpt_type, "_", time_point, "_cleaned.csv")
...
#############################
# clean clinical event data #
#############################

# get transplant date to set zero date
trnsplnt_data <- read.csv(file.path(path, "dataset/raw/Roadmap 2.0 BMT_roadmapIDs.csv")) %>%
...
for (file_name in target_data_file_list) {
  ...
  target_data <- read.csv(file.path(path, "dataset/raw", file_name)) %>%
    mutate(
  ...
  save_path <- paste0(save_folder, "/", tolower(sub("^[^_]*_", "", substr(file_name, 1, nchar(file_name)-4))), "_censored.csv")
  write.csv(target_data, save_path, row.names = FALSE, na = "")
  print(save_path)
  print(c("Unique id count", length(unique(target_data$STUDY_PRTCPT_ID))))
  print(c("All rows count", nrow(target_data)))
  # View(target_data)
}
