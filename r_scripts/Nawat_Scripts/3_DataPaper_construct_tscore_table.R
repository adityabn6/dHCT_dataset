...
save_folder <- file.path(path, "cleaned/OGPaper")
...
for (PROMIS in PROMIS_all) {
  ...
  for (prtcpt_type in c("Patients", "Caregivers")) {
    for (time_point in c("Baseline", "Day30", "Day120")) {
      ...
      data_path <- paste0(
        path, "/cleaned/Roadmap_2_", prtcpt_type, "_", time_point, "_cleaned.csv"
      )
  ...
  write.csv(
    combined_data,
    paste0(
      save_folder, "/cleaned/PROMIS raw score/Roadmap_BMT_",
...
tscore_folder_path <- paste0(path, "/cleaned/PROMIS t score")
...
for (PROMIS_score in names(PROMIS_score_dict)) {
  ...
  if (nrow(combine_data) == 0) {
    ...
    combine_data <- merge(combine_data, temp_data)
  }
}

View(combine_data)
write.csv(combine_data,
  paste0(path, "/cleaned/PROMIS_tscore_censored.csv"),
  row.names = FALSE, na = ""
)
...
View(combine_wide_data)
View(combine_long_data)
write.csv(combine_wide_data,
  paste0(path, "/cleaned/Roadmap_BMT_rawtscore_wide.csv"),
  row.names = FALSE, na = ""
)
write.csv(combine_long_data,
  paste0(path, "/cleaned/Roadmap_BMT_rawtscore_long.csv"),
  row.names = FALSE, na = ""
)
